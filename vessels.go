...
func ProcessVessels(mtDB, tjDB *Store, vessels [][]byte, db *rtree.RTree) {
	...
	for _, key := range vessels {
		...
		trajectories = CompseTraj(trajectories, db)
	}
	bar.FinishPrint("done!")
}

func CompseTraj(trajectories [][]*Obj, db *rtree.RTree) [][]*Obj {
	var comp = make([][]*Obj, 0)
	...
	for len(trajectories) > 0 {
		...
		if state == Drop {
			...
			var _ab = Extend(_a, _b)
			UnShiftQueue(trajectories, _ab)
		}
	}
	return comp
}
